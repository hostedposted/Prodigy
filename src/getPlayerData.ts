...
export const getPlayerData = async (token: string, userID?: string, { log }: { log?: boolean } = {}) => {
    if (userID) {
        return await (await fetch(`https://api.prodigygame.com/game-api/v2/characters/${userID}?fields=inventory%2Cdata%2CisMember%2Ctutorial%2Cpets%2Cencounters%2Cquests%2Cappearance%2Cequipment%2Chouse%2Cachievements%2Cstate&userID=${parseJWT(token).content.userID}`, {
            headers: {
